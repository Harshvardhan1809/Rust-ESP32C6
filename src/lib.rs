...
pub fn run(){
    ...
    // initialize devices
    let mut ina = SyncIna219::new(&mut i2c_driver_ina, Address::from_byte(0x40).expect("Out of bounds")).expect("INA219 Not detected");  
    let mut delay = Delay::new(10);
    let mut mpu = Mpu6050::new(&mut i2c_driver_ina);
